...
const gpioFunctions = [
  { text: 'Brightness UP', value: 0 },
  { text: 'Brightness Down', value: 1 },
  { text: 'Next Channel', value: 2 }
]
...
export default {
  install: (app) => {
    app.config.globalProperties.$appName = 'HV LED Driver App'
    app.config.globalProperties.$appDate = parseDate(document.documentElement.dataset.buildTimestampUtc)
    app.config.globalProperties.$ledColorOptions = ledColorOptions
    app.config.globalProperties.$ledGammaOptions = ledGammaOptions
    app.config.globalProperties.$ledScheduleOptions = ledScheduleOptions
    app.config.globalProperties.$tzOptions = tzOptions
    app.config.globalProperties.$mqttOptions = mqttOptions
    app.config.globalProperties.$gpioFunctions = gpioFunctions
  }
}
