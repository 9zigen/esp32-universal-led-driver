...
//  set_time_from_stm();
  ...
  /* Mcu temp sensor */
  system_status.mcu_temp = (double) read_mcu_temperature();
...
//  print_time();
  ...
  /* ADC: NTC + VCC */
  ESP_LOGI(TAG, "ntc temp   : %d", read_ntc_temperature());
  ESP_LOGI(TAG, "mcu temp   : %.2f", system_status.mcu_temp);
  ESP_LOGI(TAG, "power in   : %.2f", read_vcc_voltage());
  ESP_LOGI(TAG, "current    : %.2f", read_load_current());
