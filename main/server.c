...
//  if (! req->sess_ctx) {
//    ESP_LOGI(TAG, "/api/status GET allocating new session");
//    req->sess_ctx = malloc(sizeof(int));
//    req->free_ctx = session_free_func;
//    *(int *)req->sess_ctx = 0;
//  }
//
//  ESP_LOGI(TAG, "/api/status GET handler send %d", *(int *)req->sess_ctx);
...
static esp_err_t auth_post_handler(httpd_req_t *req)
{
    ...
    if (user_valid) {
        /* create new token */
        auth_generate_token(auth_token);
...
char * get_status_json()
{
    ...
    switch (chip_model) {
        case 1:
            sniprintf(name, 12, "%s", "ESP32");
            break;
        case 2:
            sniprintf(name, 12, "%s", "ESP32-S2");
            break;
        case 5:
            sniprintf(name, 12, "%s", "ESP32-C3");
            break;
        case 9:
            sniprintf(name, 12, "%s", "ESP32-S3");
            break;
        case 12:
            sniprintf(name, 12, "%s", "ESP32-C2");
            break;
        case 13:
            sniprintf(name, 12, "%s", "ESP32-C6");
            break;
        case 16:
            sniprintf(name, 12, "%s", "ESP32-H2");
            break;
        default:
            sniprintf(name, 12, "%s", "ESP32-NA");
            break;
    }
    ...
    cJSON_AddItemToObject(status, "hardware", cJSON_CreateString(name));
