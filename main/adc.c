...
#define CUR_ADC ADC_CHANNEL_0  /* GPIO0 */
#define VCC_ADC ADC_CHANNEL_1  /* GPIO1 */
#define NTC_ADC ADC_CHANNEL_3  /* GPIO3 */

#define ADC_SENSORS_NUM 3
#define ADC_SENSORS { CUR_ADC, VCC_ADC, NTC_ADC }
#endif
...
#define VCC_DIVIDER_RATIO                27.6f /* 26.68 calibrated value if 49v_in 1.77v_adc*/
...
void init_adc()
{
    ...
    for (int i = 0; i < ADC_SENSORS_NUM; ++i) {
        do_calibration[i] = adc_calibration_init(ADC_UNIT_1, adc_sensors[i], ADC_ATTEN_DB_12, &adc_cali_handle[i]);
    }
}
...
float read_vcc_voltage()
{
    ...
    return voltage * VCC_DIVIDER_RATIO;
}
