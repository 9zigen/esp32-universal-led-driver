...
void set_token(char* token)
{
    ...
    if (err != ESP_OK) {
        ...
        /* Write */
        ESP_LOGI(TAG, "Updating Token in NVS %s [%u]...", token, strlen(token));
        err = nvs_set_str(nvs_handle, "token", token);
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "Failed to Update Token in NVS");
        } else {
            ESP_LOGI(TAG, "Done New Token");
        }
    }
...
char * load_token()
{
    ...
    size_t required_size = 0;  /* value will default to 0, if not set yet in NVS */
    err = nvs_get_str(nvs_handle, "token", NULL, &required_size);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGE(TAG, "token not saved yet! error: %s", esp_err_to_name(err));
    } else {
        char *token = calloc(1, required_size + 1);
        if (token == NULL) {
            ESP_LOGE(TAG, "Error: malloc failed");
            return NULL;
        }
        err = nvs_get_str(nvs_handle, "token", token, &required_size);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Error (%s) reading!\n", esp_err_to_name(err));
            free(token);
            return NULL;
